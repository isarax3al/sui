...
use crate::config::HashiNodeValidationConfig;
...
pub struct SuiNodeProvider {
    ...
    hashi_config: Option<HashiNodeValidationConfig>,
}
...
impl SuiNodeProvider {
    ...
    async fn update_hashi_committee_set(&self, cfg: &HashiNodeValidationConfig) {
        let validator_names: BTreeMap<SuiAddress, String> =
            self.sui_validator_names.read().unwrap().clone();

        match resolve_hashi_committee(&self.rpc_url, cfg, &validator_names).await {
            Ok(result) => {
                HASHI_OBSERVED_EPOCH
                    .with_label_values(&[&cfg.hashi_object_id])
    ...
    pub fn poll_peer_list(&self) {
        ...
        // Hashi runs on its own cadence (per HashiNodeValidationConfig.interval) so
        // operator key rotations can be picked up faster than the global poll without
        // increasing pressure on the Sui validator-set RPC.
        if let Some(cfg) = self.hashi_config.clone() {
            let cloned_self = self.clone();
            let hashi_interval = cfg.interval;
            tokio::spawn(async move {
                let mut interval = tokio::time::interval(hashi_interval);
                interval.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Skip);
                loop {
                    interval.tick().await;
                    cloned_self.update_hashi_committee_set(&cfg).await;
                }
            });
        }
    }
...
//     ...                                 // BLS / encryption keys (irrelevant here)
//
//   Committee
...
//   CommitteeMember
//     validator_address: address
//     ...                                 // BLS / weight (irrelevant here)
//
// Resolver flow per poll:
//   1. sui_getObject(Hashi)                  -> epoch, pending_epoch, members bag id, committees bag id
//   2. suix_getDynamicFieldObject(committees bag, key = current_epoch)
//      [+ pending_epoch if present]          -> validator_address list
//   3. for each validator_address:
//      suix_getDynamicFieldObject(members bag, key = address)
//                                            -> MemberInfo.tls_public_key
//
// Members with an unset (empty / non-32-byte) tls_public_key are skipped — they
// haven't completed registration yet and can't authenticate anyway.
...
async fn resolve_hashi_committee(
    rpc_url: &str,
    cfg: &HashiNodeValidationConfig,
    validator_names: &BTreeMap<SuiAddress, String>,
) -> Result<HashiResolution> {
    let snapshot = get_hashi_committee_snapshot(rpc_url, &cfg.hashi_object_id).await?;
    debug!(
    ...
    // Collect the union of validator_addresses across the active and pending
    // committees. Using a set tolerates the (expected) overlap during reconfig.
    //
    // We tolerate a missing current Committee: at hashi genesis the CommitteeSet
    // starts with `epoch=0` and an empty `committees` Bag, and the first
    // Committee only materializes once `start_reconfig` runs. During that
    // pre-genesis window the resolver should keep polling and produce an empty
    // allowlist (not bail), so the proxy is ready the moment hashi members
    // start registering.
    ...
    if let Some(next) = snapshot.pending_epoch {
        match get_committee_validator_addresses(rpc_url, &snapshot.committees_bag_id, next).await {
            ...
            // Pending committee should normally exist when pending_epoch_change is Some,
            // but if there's a transient mismatch we log and continue rather than fail
...
/// Pure extraction step: filter to members with valid 32-byte tls_public_key
/// and build AllowedPeer entries with a `hashi-<validator name>` label. Kept
/// pure so it can be exhaustively unit-tested without an RPC server.
fn extract_hashi(
...
async fn get_hashi_committee_snapshot(
    ...
    let fields = body
        ...
        .ok_or_else(|| anyhow!("missing committee_set fields in Hashi object response"))?;
...
async fn get_committee_validator_addresses(
    ...
    let committee_fields = body
        ...
        .ok_or_else(|| {
            anyhow!("missing Committee fields for epoch {epoch} under {committees_bag_id}")
        })?;

    let members = committee_fields
        .get("members")
        .and_then(Value::as_array)
        .ok_or_else(|| anyhow!("Committee.members is not an array for epoch {epoch}"))?;
    ...
    for m in members {
        let addr_str = m
            ...
            .ok_or_else(|| anyhow!("missing CommitteeMember.validator_address"))?;
...
async fn get_hashi_member_info(
    ...
    let member_fields = body
        .pointer("/result/data/content/fields/value/fields")
        .ok_or_else(|| {
            anyhow!("missing MemberInfo fields for validator {validator_address} under {members_bag_id}")
        })?;
...
/// Helper: parses Move's `Bag { id: UID { id: <ObjectID> }, size: u64 }` shape
/// from a Sui JSON-RPC display response and returns the inner ObjectID as a String.
fn parse_bag_id(field: Option<&Value>, name: &str) -> Result<String> {
    field
        .and_then(|f| f.pointer("/fields/id/id"))
        .and_then(Value::as_str)
        .map(|s| s.to_owned())
        .ok_or_else(|| anyhow!("could not extract {name}.id from JSON-RPC response"))
}
    ...
    if let Some(arr) = value.pointer("/vec").and_then(Value::as_array) {
        return match arr.first() {
            Some(v) => {
                let s = v
                    .as_str()
                    .ok_or_else(|| anyhow!("Option<u64>.vec[0] not a string"))?;
...
/// Tiny helper around the existing JSON-RPC pattern used elsewhere in this file.
/// Returns the raw response Value so callers can pointer-walk into nested fields
/// without imposing a fixed schema (the Move display layout differs per type).
...
mod tests {
    ...
    fn extract_hashi_skips_members_with_empty_tls_key() {
        ...
        assert_eq!(peers.len(), 1, "only the member with a 32-byte key survives");
    }
    ...
    fn parse_bag_id_extracts_inner_id() {
        let v: Value = serde_json::from_str(
            r#"{ "fields": { "id": { "id": "0xdeadbeef" }, "size": "5" } }"#,
        )
        .unwrap();
