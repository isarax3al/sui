...
impl AV::Visitor<'_> for AddressExtractor {
    type Value = Option<AccountAddress>;
    type Error = FormatError;

    visitor_default! { <'_> u8, u16, u32, u64, u128, u256 = Ok(None) }
    visitor_default! { <'_> bool, signer, vector, variant = Ok(None) }
    ...
    fn visit_struct(
        &mut self,
        driver: &mut AV::StructDriver<'_, '_>,
    ) -> Result<Self::Value, Self::Error> {
        let id = ID::compressed_layout();
        ...
        let Some((field_name, field_layout)) = driver.peek_field() else {
            return Ok(None);
        };
        ...
        let CA::MoveLayoutView::Struct(field) = field_layout.as_view() else {
            return Ok(None);
        };
...
fn extract_address(driver: &mut AV::StructDriver<'_, '_>) -> Result<AccountAddress, FormatError> {
    driver.skip_field()?;
