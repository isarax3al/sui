...
impl ValidatorSetContents {
    ...
    fn deserialize(
        ...
        impl<'b> AV::Traversal<'b> for Traversal<'_, 'b> {
            type Error = Error;
                ...
                while let Some((name_ref, field_layout)) = driver.peek_field() {
                    ...
                    match self {
                        Traversal::SystemState(c) if name == "validators" => {
                            ...
                            let bytes = &driver.bytes()[lo..hi];
                            c.native = Some((bytes, field_layout));
                        }
        ...
        impl AV::Visitor<'_> for AddressVisitor {
            type Value = Option<NativeSuiAddress>;
            type Error = Error;

            visitor_default! { <'_> u8, u16, u32, u64, u128, u256 = Ok(None) }
            visitor_default! { <'_> bool, signer, vector, struct, variant = Ok(None) }
