...
use crate::{
    ...
    annotated_value::{
        self as A, MoveFieldLayout, MoveStruct, MoveStructLayout as TreeStructLayout, MoveValue,
        MoveVariant,
    },
    annotated_visitor::{
    ...
    compressed::annotated::{MoveLayoutView, MoveStructLayout, MoveTypeLayout},
...
impl<'b> Traversal<'b> for CountingTraversal {
    ...
    fn traverse_u8(&mut self, _driver: &ValueDriver<'b>, _value: u8) -> Result<(), Self::Error> {
        self.0 += 1;
        Ok(())
    }

    fn traverse_u16(&mut self, _driver: &ValueDriver<'b>, _value: u16) -> Result<(), Self::Error> {
        self.0 += 1;
        Ok(())
    }

    fn traverse_u32(&mut self, _driver: &ValueDriver<'b>, _value: u32) -> Result<(), Self::Error> {
        self.0 += 1;
        Ok(())
    }

    fn traverse_u64(&mut self, _driver: &ValueDriver<'b>, _value: u64) -> Result<(), Self::Error> {
        self.0 += 1;
        Ok(())
    }
...
impl<'b> Visitor<'b> for PrintVisitor {
    ...
    fn visit_vector(&mut self, driver: &mut VecDriver<'_, 'b>) -> Result<Self::Value, Self::Error> {
        let layout = driver.element_layout().inflate().unwrap();
    ...
    fn visit_struct(
        ...
        while let Some((name, value)) = driver.next_field(&mut field_visitor)? {
...
fn traversal() {
    ...
    let tree_layout = struct_layout_(
        "0x0::foo::Bar",
        vec![
            ("a", T::U8),
            ("b", T::U16),
    ...
    let struct_layout = struct_only_(&type_layout);
...
fn unexpected_eof() {
    ...
    let tree_layout = struct_layout_("0x0::foo::Bar", vec![("a", T::U64)]);
...
fn no_enum_tag() {
    use A::MoveTypeLayout as T;
    use MoveValue as V;
    let tree_layout = enum_layout_("0x0::foo::Bar", vec![("a", vec![("b", T::U8)])]);
...
#[test]
fn bad_bool_byte() {
    let layout: MoveTypeLayout = (&A::MoveTypeLayout::Bool).try_into().unwrap();
    assert_eq!(
        "unexpected byte: 42",
        MoveValue::visit_deserialize(&[42], layout, &mut NullTraversal)
            .unwrap_err()
            .to_string(),
    );
}
...
#[test]
fn trailing_bytes() {
    let layout: MoveTypeLayout = (&A::MoveTypeLayout::U8).try_into().unwrap();
    assert_eq!(
        "trailing 1 byte(s) at the end of input",
        MoveValue::visit_deserialize(&[42, 42], layout, &mut NullTraversal)
            .unwrap_err()
            .to_string(),
    );
}
...
fn peek_field_test() {
    ...
    impl<'b> Visitor<'b> for PeekU64Visitor<'_> {
        ...
        fn visit_struct(
            ...
            while let Some((name, _)) = driver.peek_field() {
        ...
        fn visit_u8(&mut self, _: &ValueDriver<'b>, _: u8) -> Result<Self::Value, Self::Error> {
            Ok(None)
        }

        fn visit_u16(&mut self, _: &ValueDriver<'b>, _: u16) -> Result<Self::Value, Self::Error> {
            Ok(None)
        }

        fn visit_u32(&mut self, _: &ValueDriver<'b>, _: u32) -> Result<Self::Value, Self::Error> {
            Ok(None)
        }

        fn visit_u128(&mut self, _: &ValueDriver<'b>, _: u128) -> Result<Self::Value, Self::Error> {
            Ok(None)
        }

        fn visit_u256(&mut self, _: &ValueDriver<'b>, _: U256) -> Result<Self::Value, Self::Error> {
            Ok(None)
        }

        fn visit_bool(&mut self, _: &ValueDriver<'b>, _: bool) -> Result<Self::Value, Self::Error> {
            Ok(None)
        }
...
fn byte_offset_test() {
    ...
    impl<'b> Traversal<'b> for ByteOffsetVisitor {
        ...
        fn traverse_u8(&mut self, driver: &ValueDriver<'b>, value: u8) -> Result<(), Self::Error> {
            write!(
                &mut self.0,
                "\n[{:>3} .. {:>3}] {value}: u8",
                driver.start(),
        ...
        fn traverse_vector(&mut self, driver: &mut VecDriver<'_, 'b>) -> Result<(), Self::Error> {
            write!(
                ...
                driver.start(),
                driver.position(),
                driver.element_layout().inflate().unwrap(),
            )
            .unwrap();
            while driver.next_element(self)?.is_some() {}
            Ok(())
...
/// Create a tree-form struct layout for test purposes.
pub(crate) fn struct_layout_(
    rep: &str,
    ...
    A::MoveTypeLayout::Struct(Box::new(TreeStructLayout { type_, fields }))
}
...
/// Create a tree-form enum layout for test purposes.
pub(crate) fn enum_layout_(
    rep: &str,
    variants: Vec<(&str, Vec<(&str, A::MoveTypeLayout)>)>,
) -> A::MoveTypeLayout {
    let type_ = StructTag::from_str(rep).unwrap();
